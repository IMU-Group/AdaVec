...
def main(dataset_path,path):
    os.makedirs(f"/root/autodl-tmp/img2vec/metrics/results/live/path_{path}", exist_ok=True)
    for dataset_name in os.listdir(dataset_path):
        os.makedirs(os.path.join(f"/root/autodl-tmp/img2vec/metrics/results/live/path_{path}", dataset_name), exist_ok=True)
        log = {}
        count = 0
        ...
        for image_name in os.listdir(os.path.join(dataset_path, dataset_name)):
            ...
            predict_svg_path = glob.glob(os.path.join(f"/root/autodl-tmp/SGLIVE/SGLIVE/test/result/live_{path}", dataset_name, image_name,
                                            f"output-svg/{path}_*.svg"))
            ...
            predict_image_path = os.path.join(f"/root/autodl-tmp/img2vec/metrics/results/live/path_{path}", dataset_name, image_name)
            ...
            json_file_path=os.path.join(f"/root/autodl-tmp/SGLIVE/SGLIVE/test/result/live_{path}", dataset_name, image_name,"log.json")
        ...
        with open(os.path.join(f"/root/autodl-tmp/img2vec/metrics/results/live/path_{path}", dataset_name, "log.json"), 'w', encoding='utf-8') as f:
