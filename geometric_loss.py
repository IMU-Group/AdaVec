...
class GeometryLoss:
    ...
    def compute(self, pathObjs):
        ...
        # Smoothness
        max_len = max([pathObj.points.shape[0] for pathObj in pathObjs])+1
        curves = []
        for pathObj in pathObjs:
            if pathObj.points.shape[0] < max_len:
                pathObj_points = pathObj.points.to(self.device).view(-1, 2)
                pathObj_pad = pathObj.points[:1, :].detach().repeat(max_len - pathObj.points.shape[0], 1)
                curves.append(torch.cat((pathObj_points, pathObj_pad), dim=0))
            else:
                curves.append(pathObj.points.to(self.device).view(-1, 2))
    ...
    def control_geometric_loss(self, curves, temperature=10):  # x[npoints,2]
        ...
        AB = (A - B) / (torch.norm((A - B)+1e-5, dim=-1)[..., None])
        BC = (B - C) / (torch.norm((B - C)+1e-5, dim=-1)[..., None])
        CD = (C - D) / (torch.norm((C - D)+1e-5, dim=-1)[..., None])
        dot_product = dot(AB, CD)
        cross_product = cross(AB, CD)
        signed_angle = torch.atan2(cross_product++1e-5, dot_product)
